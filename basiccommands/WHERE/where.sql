create database app;
use app;
CREATE TABLE rides (ride_id INT,driver_id INT,rider_id INT,pickup_location VARCHAR(100),dropoff_location VARCHAR(100),ride_date DATETIME,fare DECIMAL(10, 2));
...
select * from  rides;
select * from  rides where fare;
select * from  rides order by fare;
select * from  rides where fare>300 and pickup_location='Chennai';
desc rides;
